...
from dnn.metrics import regression_metrics  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train regression model (games differential).")
...
def train_one_epoch(model: nn.Module, loader: DataLoader, device: torch.device, criterion: nn.Module, optimizer: torch.optim.Optimizer) -> float:
    ...
    for xb, yb in loader:
        xb = xb.to(device)
        # Targets are (batch, num_outputs); keep shape.
        yb = yb.to(device)
        optimizer.zero_grad()
        preds = model(xb)
...
def main() -> None:
    ...
    splits = build_splits(args.data, target_prefix="reg")
    train_loader, val_loader, test_loader = make_loaders(splits, batch_size=args.batch_size)

    input_dim = splits.X_train.shape[1]
    output_dim = splits.y_train.shape[1] if splits.y_train.ndim > 1 else 1
    model = build_mlp(input_dim=input_dim, output_dim=output_dim, model_size=args.model_size).to(device)
    ...
    model_path = paths.models / f"reg_{args.model_size}.pt"
    torch.save(
        {"model_state": model.state_dict(), "input_dim": input_dim, "model_size": args.model_size, "output_dim": output_dim},
        model_path,
    )
    ...
    report_path = paths.reports / f"regression_{args.model_size}.json"
